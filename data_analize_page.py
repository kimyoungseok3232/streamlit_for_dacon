...
# 페이지에 있는 이미지 출력
def show_images(type, data, window):
    cols = window.columns(5)
    ex = ex = ['ID', 'label', 'index']
    imgs = data[[col for col in data.columns if col not in ex]].values.reshape(-1,32,32)
...
def show_dataframe(dataset,window,type):
    ...
    show_images(type, pages[current_page - 1], con2)
...
if option == "원본 데이터":
    ...
    if not targetdata:
        ...
        target = st.sidebar.selectbox("타겟",traind['label'].unique())
...
# output 파일 체크
    ...
    outdata = output_data[selected_file][['ID', 'label']]
    outdata = pd.merge(outdata, testd, on='ID', how='inner')
    # 데이터 출력
    ...
    if not targetdata:
        ...
        outtargetcount = outdata["label"].value_counts().sort_index()
        ...
        target = st.sidebar.selectbox("타겟",outdata['label'].unique())
        outdata = outdata.loc[outdata.label == target].reset_index(drop=True)
        show_dataframe(outdata,st,folder)